...
> 为控制插件体积，本插件**不打包**优秀获奖论文 PDF。撰写论文前建议用户自备
> 优秀论文（CUMCM 国赛优秀论文、MCM/ICM O 奖论文可在官网及各高校资源站获取），
> 放入项目目录（如 `<项目>/参考论文/`）后按下面方式使用。
...
```
当需要参考优秀论文时，使用 tools/pdf skill 读取用户提供的PDF文件：

1. 询问用户是否有可参考的优秀论文，确定需要参考的论文类型和题目
